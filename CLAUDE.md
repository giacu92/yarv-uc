...
- Generated clock `clk_core` at 100 MHz on the `clk_core` net (`create_generated_clock -source clk_i -master_clock clk25 -multiply_by 4 -divide_by 1`; 25×20/5, period 10 ns). The whole fabric runs on `clk_core`, so this is the only timing-critical clock; `clk25` only feeds the rPLL and has no user-logic paths.
...
- `cmd.do` passes `-sdc src/phys/rv32imac_Zicsr_Zifencei.sdc` and sets `global_freq 100.000` MHz (matching the 100 MHz `clk_core`); the 25 MHz `clk25` only feeds the rPLL. Verify timing closure against `clk_core` before relying on it.
