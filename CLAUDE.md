...
**Timing (2026-08-25, post-LSU-register-stage, 16 KiB memories, UART + xbar_3 in build):** 25 MHz PLL-bypass closes with **+10.152 ns worst setup slack** (40 ns period, 29.813 ns data delay → ~33.5 MHz Fmax), hold min +0.213 ns. Critical path is `u_cpu/u_decode/de_q.csr_addr_0_s0/Q → u_cpu/fetch_stage_i/pc_q_*/D` — the CSR-address fan-out into the redirect, which is where it sat historically; **the LSU is out of the picture**. Progression: +2.2 ns (pre-fix, ~27.7 MHz) → +5.320 ns (byte offset latched, ~28.9 MHz, limiter = store data path into `mtimecmp`) → +10.152 ns (request registered). The 35 MHz rPLL retarget was verified at 35.049 MHz / +0.040 ns back on 2026-08-24 and there is now real headroom for it, but it has not been re-run since. A safe 40 MHz still wants the async CSR read pipelined — that is the limiter again. Legacy `VON_NEUMANN` build dropped — Harvard only.
