...
- `sim/sim_top.sv` — sim wrapper. Replicates `top_module`'s wiring (CPU `imem_axi` → `axi4_lite_ram`; `peri_axi` tied off) but instantiates the RAM with `INIT_FILE="program.hex"` so a program is preloaded via `$readmemh`, and exposes the CPU's full `fe_*` (fetch) AND `de_*` (decode) debug taps as output ports for the C++ log.
- `sim/sim_main.cpp` — drives clk/rst (async active-low reset for a few cycles), clocks the design, and prints **two** logs: a **fetch log** (one line per F/D-valid word: `cycle / fe_pc / fe_instr / c / fe_next_pc`) and a **decode log** (one line per D/E-valid instruction: `cycle / de_pc / C / de_instr / opc / rd / rs1 / rs2 / imm / r1d / r2d / rw / alu / br / mr / mw / ms / wu / wb / ill`). Each log is its own reset+run pass so the two stay aligned. A VCD (`sim/sim_top.vcd`) is written for GTKWave.
...
Fetch and decode are implemented but there is **no execute stage**, so instructions do **not** execute — the sim verifies fetch delivers the right word at the right PC (advancing by 4), with the right `fe_is_compressed` flag, that the AXI4-Lite round-trip behaves, and that decode produces the right control + immediates + reg reads (reads are 0 until a writeback stage exists). Requires Verilator installed on the build host:
...
- **Pipeline debug taps (stage sigil):** every pipeline stage exposes the PC it is treating, the instruction word, a valid, and any other useful debug as outputs prefixed by a stage-identifying sigil — `fe_` (fetch), `de_` (decode), `ex_` (execute, future), .... The sigil names the **producing** stage: the F/D register is fetch's output (`fe_pc`/`fe_instr`/`fe_valid`/`fe_is_compressed`/`fe_next_pc`); the D/E register is decode's output (`de_pc`/`de_instr`/`de_valid`/...). A stage's **consuming** input ports take the producer's sigil (decode's F/D inputs are `fe_*_i`). The CPU top mirrors these as `fe_*_dbg_o` / `de_*_dbg_o` flat-logic taps (plain `logic`, not enum-typed, so Verilator C++ and Gowin have no enum-in-port edge cases); the board top leaves them unconnected (swept by synthesis) except `fe_pc_dbg_o[3:0]` → LEDs.
...
- **Compressed (C)**: NOT handled here — always fetches 32 bits, advances by 4; `fe_is_compressed_o = (rdata[1:0] != 2'b11)` is computed for decode, which owns RVC expansion (a compressed instr in the low half of a word ⇒ the next instr is the upper half of the same word, handled in decode).
...
- **RVC spanning (phase-1 simplification)**: fetch delivers 32-bit words at 4-byte boundaries + `fe_is_compressed`. A 32-bit instr is assumed 4-byte aligned; the spanning case (low half compressed AND upper half `[1:0]==2'b11`) is **undefined → `illegal=1`** (documented limitation). This is the only known RVC limitation today.
