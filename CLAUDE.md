...
- **`top_module.sv`** — board top. 25MHz `clk_i`→rPLL→`clk_core`=40MHz, single clock domain (no CDC). Harvard: native `u_imem` (read-only) on `CPU.imem`, native `u_dmem` (byte-strobed) on `CPU.dmem`, peri tieoff on `axi_bus_peri`. `VON_NEUMANN`: `bus_axi`→`axi4_lite_xbar`→mem (`axi4_lite_ram`) / peri. `led_o[0]`=stall, `led_o[3:1]`=free-running counter.
...
- Synth + PnR on the remote build host **confirmed** (2026-08-21): Harvard synthesizes clean (`native_ram.sv` Gowin-clean — EX3990/EX3900/BSRAM rules followed) and PnR closes the 40MHz target — Actual Fmax 40.058MHz, worst setup slack +0.037ns, TNS 0. Native I/D-mem paths are shorter than the old AXI round-trip; the execute/ALU/decode route-dominated limiter is unchanged. (Headroom note: Gowin's realized Fmax tracks the constraint effort — to see the design's true Fmax ceiling, re-run with a tighter target.)
