...
### Place & Route
Options in `impl/pnr/cmd.do` (GW2AR-18C, `-bit -tr -ph -timing`, `global_freq 40.000`); device opts in `impl/pnr/device.cfg`. Run via wrapper:
```bash
...
- **`rv32imac_zicsr_zifencei.sv`** — CPU top wiring fetch+reg_file+csr_regfile+decode+execute+**trap_unit**. Three ports (Harvard default): native `imem`/`dmem` + AXI4-Lite `axi_peri`; the peri `axi4_lite_master_bridge` lives inside the CPU. Plus `msip_i` (machine software-interrupt pending, from the board's `msip_peri` slave; tied low in VON_NEUMANN). `trap_unit` is a combinational peer of execute: execute exports sync_trap/mret/take_interrupt triggers + cause/tval/pc, imports `int_pending` + `trap_redirect_addr`; the trap unit drives the CSR trap-write bundle (back to `csr_regfile`) + the fetch redirect (merged with the normal branch on `branch_valid`/`branch_addr`). `VON_NEUMANN` collapses to one `bus_axi` master (fetch+LSU share `mem_arbiter`). Only non-bus output: `dbg_stall_o`.
- **`top_module.sv`** — board top. 25MHz `clk_i`→rPLL→`clk_core`=40MHz, single clock domain (no CDC). Harvard: native `u_imem` (read-only) on `CPU.imem`, native `u_dmem` (byte-strobed) on `CPU.dmem`, `u_msip` (`msip_peri`) on `axi_bus_peri` — its `msip_o` feeds `CPU.msip_i` (was a tieoff). `VON_NEUMANN`: `bus_axi`→`axi4_lite_xbar`→mem (`axi4_lite_ram`) / peri (tieoff — no MSIP slave in the von-Neumann board top; `msip_i` tied low). `led_o[0]`=stall, `led_o[3:1]`=free-running counter.
...
- **`msip_peri.sv`** (utils) — AXI4-Lite MMIO slave, 1-bit MSIP register at `MSIP_PERI_ADDR` (peri base `0x1000_0000`). Write bit[0] sets/clears `mip.MSIP`; reads return `{31'b0, msip}`; `msip_o` → `csr_regfile.msip_i`. Protocol mirrors `axi4_lite_ram` (registered BVALID held until B handshake, RVALID held until RREADY, single-outstanding). The only peri slave for now (a UART/GPIO drops in alongside via an address-decoded mux).
...
- Need a real UART/GPIO slave on `axi_bus_peri` alongside `msip_peri` (decode path — `addr[PERI_ADDR_BIT]` steer — is already in place; add an address-decoded peri mux when a second slave lands).
...
- `cmd.do` targets 40MHz (was 50MHz; backed off because the route-dominated execute/ALU/decode path closed with only ~0.06ns slack at 50MHz — noisy run-to-run). At the 40MHz target PnR closes 40.058MHz Actual Fmax, worst setup slack +0.037ns, TNS 0 (regfile primitive already ruled out as limiter — route-dominated execute/ALU/decode path is the next target for headroom).
