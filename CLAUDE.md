...
RV32IMAC + Zicsr + Zifencei RISC-V core for a **Gowin GW2AR-18C** FPGA (QFN88) on a Tang Nano 20k. Clock: 25 MHz single-ended from an MS5351M generator (CLK0, PIN10, LVCMOS33) — not the stock 27 MHz oscillator. **Active build: 40 MHz via on-chip rPLL** (`clk_core = clk_i × 8/5`, IDIV_SEL=4 / FBDIV_SEL=7 / ODIV_SEL=16, VCO 640 MHz, period 25 ns) — a **target, not yet a verified closure**: it follows the LSU and CSR register stages, which took the 25 MHz bypass build from +2.2 to +10.152 ns of slack (~33.5 MHz Fmax) and then removed the CSR-address fan-out that limited it. The next PnR run is what says whether 25 ns holds. Fallback is three lines in `top_module` plus re-commenting the SDC generated clock and setting `global_freq 25.000` (recipe in the file). The 25 MHz bypass is also the diagnostic config: it removes the rPLL from the picture entirely.
...
**Timing (2026-08-25, latest measured build = 25 MHz bypass with the LSU + CSR register stages):** +12.577 ns worst setup slack, 27.388 ns data delay → **~36.5 MHz Fmax**, hold min +0.225 ns. Critical path is `u_cpu/u_regfile/regs_regs_0_0_s/DO[26] → u_cpu/fetch_stage_i/pc_q_*/D`: regfile read → forward mux → branch compare / target → PC redirect. Slack progression, all at 25 MHz: +2.2 ns (nothing done, ~27.7 MHz) → +5.320 ns (load byte offset latched, ~28.9 MHz, limiter the store path into `mtimecmp`) → +10.152 ns (LSU request registered, ~33.5 MHz, limiter the CSR-address fan-out) → +12.577 ns (CSR read registered, ~36.5 MHz, limiter the branch/redirect path). **The active build now targets 40 MHz via the rPLL** — 25 ns against a measured 27.388 ns, so it does not obviously fit; the measurement was taken under a 40 ns constraint, though, so PnR had no reason to push and a run constrained at 25 ns is the only way to know. If it misses, 35 MHz (28.571 ns) already clears today's number with ~1.2 ns to spare, and the thing to attack for 40 is that branch path: either compute PC-relative targets in decode (only `jalr` actually needs a register operand) or register the redirect, which costs a cycle on taken branches. Legacy `VON_NEUMANN` build dropped — Harvard only.
...
- `cmd.do` targets **40 MHz** (history 50→40→25 bypass→35 rPLL→25 bypass→40 rPLL). The two register stages that made this plausible are in: the LSU request (+4.8 ns) and the CSR read (which removed the limiter the last report named). Unverified until the next PnR run.
