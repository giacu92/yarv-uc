...
**Timing (2026-08-25, post-LSU-fix, 16 KiB memories, UART + xbar_3 in build):** 25 MHz PLL-bypass closes with **+5.320 ns worst setup slack** (40 ns period, 34.645 ns data delay → ~28.9 MHz Fmax), hold min +0.316 ns. Critical path is now the **store data path into the CLINT**: `u_cpu/u_regfile/regs_regs_0_0_s/DO[1] → u_timer/mtimecmp_q_*/D` — regfile read → store pre-shift → peri bus → xbar → `mtimecmp`. Earlier history: pre-forwarding the limiter was the route-dominated CSR-address fan-out (~37 MHz actual); post-forwarding it was the regfile async read → decode forward mux, and the 35 MHz rPLL retarget closed knife-edge at 35.049 MHz / +0.040 ns (2026-08-24). If the store path ever needs shortening the cheap moves are local: register address+data inside `clint_timer` (a skid in the slave, no CPU impact), or skip the store pre-shifter for `MS_W` where the shift is always zero. A pipeline stage in the LSU is NOT indicated at +5.3 ns of margin. Legacy `VON_NEUMANN` build dropped — Harvard only.
