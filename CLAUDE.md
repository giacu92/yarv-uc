...
A RV32IMAC + Zicsr + Zifencei RISC-V core targeting a **Gowin GW2AR-18C** FPGA (part `GW2AR-LV18QN88C8/I7`, QFN88 package), on a Tang Nano 20k-based board. The board is clocked by a **25 MHz differential reference** from an **MS5351M** clock generator (crystal-fed) on **PIN10 (P) / PIN11 (N)**, Bank 6 — not the stock 27 MHz oscillator. The core is a work-in-progress pipeline: **fetch and decode stages are implemented** (the pipe reaches the D/E register); **execute onward is not yet present**. The CPU exits **two AXI4-Lite masters** (`imem_axi`, `peri_axi`). The native `mem_req_t` / `mem_rsp_t` → AXI4-Lite conversion is done **inside the CPU** (one `axi4_lite_master_bridge` per master port), so the rest of the system sees the CPU as a plain AXI4-Lite master and the board top stays free of glue.
...
Outputs `impl/pnr/rv32imac_Zicsr_Zifencei.fs` / `.bin` for programming. `cmd.do` passes `-sdc src/phys/rv32imac_Zicsr_Zifencei.sdc`, so the timing constraints are applied, and sets `global_freq 100.000` to match the 100 MHz `clk_core`.

### Constraints (in `src/phys/`, NOT `impl/pnr/`)

Pin assignment — `src/phys/rv32imac_Zicsr_Zifencei.cst`:
- `clk_p_i` / `clk_n_i` → PIN10 / PIN11 (25 MHz differential, MS5351M clock generator, crystal-fed; Bank 6, `IO_TYPE=LVDS25`, taken through a `TLVDS_IBUF` in `top_module`. Bank 6 has **no on-chip 100R differential termination** — the board must fit an external 100R across P/N.)
...
- Primary clock `clk25` at 25 MHz on `clk_p_i` (period 40 ns).
- Generated clock `clk_core` at 100 MHz on the `clk_core` net (`create_generated_clock -source clk_p_i -master_clock clk25 -multiply_by 4 -divide_by 1`; 25×20/5, period 10 ns). The whole fabric runs on `clk_core`, so this is the only timing-critical clock; `clk25` only feeds the rPLL and has no user-logic paths.
...
- `cmd.do` passes `-sdc src/phys/rv32imac_Zicsr_Zifencei.sdc` and sets `global_freq 100.000` MHz (matching the 100 MHz `clk_core`); the 25 MHz differential `clk25` only feeds the rPLL. Verify timing closure against `clk_core` before relying on it.
